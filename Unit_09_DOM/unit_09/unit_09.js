...
function f6() {
    ...
    let a = document.querySelectorAll('.p-6').length; // это массив, высчитываем его длину через length
...
//  Task 9
// Усложним предыдущие задачи. С помощью цикла повесьте на блоки out-9 событие клик. По клику должна выполняться функция f9. 
// Функция, должна добавлять класс bg-9 тому out-9 на котором кликнули.

function f9() {
    this.classList.add('bg-9'); // присваиваем out-9 класс bg-9
}
...
//  Task 10
// Усложним предыдущие задачи. С помощью цикла повесьте на блоки out-10 событие клик. По клику должна выполняться функция f10. 
// Функция, должна делать toggle класса bg-10 тому out-10 на котором кликнули.
...
//  Task 13
// Добавьте кнопку .b-13, которая запускает функцию f13. Функция создает через createElement div c текстом pushMe и добавляет ему класс bg-13. 
// Также, созданному div добавляется событие onclick, по которому выполняется функция f13_1. Созданный div добавляется в out-13.

let out13 = document.querySelector('.out-13');

function f13() {
    let a = document.createElement('div'); // создаем класс div
    a.textContent = 'pushMe'; // добавляем текст в div
    a.classList.add('bg-13'); // добавляем класс bg-13 в класс div
    a.onclick = function f13_1() { }; // вешаем событие onclick на класс div и приравниваем функцию
    out13.appendChild(a); // добавляем класс div в out-13
...
//  Task 14
// Добавьте кнопку .b-14, которая запускает функцию f14. Функция создает через createElement div c текстом 14 и добавляет ему класс bg-14. 
...
//  Task 15
// Добавьте кнопку .b-15, которая запускает функцию f15. Функция создает через createElement div c текстом 15 и добавляет ему класс bg-15. 
...
function f15() {
    ...
    out15.before(a);   // Метод .before() добавляет текст перед заданным элементом.
}
...
function f16() {
    ...
    out16.after(a); // Метод .after() добавляет текст после заданного элемента.
...
function f17() {
    ...
    out17.replaceWith(a); // Метод .replaceWith() заменяет одни элементы другими. 
...
//  Task 19
// Добавьте кнопку .b-19, которая запускает функцию f19. Функция с помощью getAttribute получает data-b атрибут с параграфов p-19 и выводит в out-19 через пробел. 
